...
const App: React.FC = () => {
  ...
  const parseVoxelResponse = (text: string): VoxelData[] => {
  ...
  const handlePromptSubmit = async (prompt: string) => {
    ...
    try {
        ...
        if (response.text) {
            const voxelData = parseVoxelResponse(response.text);
  ...
  const handleImageSubmit = async (file: File, userPrompt: string) => {
    if (!process.env.API_KEY) {
        throw new Error("API Key not found");
    }
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: {
                parts: [
                    ...
                    {
                        text: `
                            Task: Analyze this image and generate a 3D voxel art model of the main subject.
                            ...
                            2. The model must be centered at x=0, z=0.
                            3. The bottom of the model must be at y=0 or slightly higher.
                            4. Capture the colors from the image accurately.
                            5. Ensure the structure is physically plausible (connected).
        ...
        if (response.text) {
            const voxelData = parseVoxelResponse(response.text);
            const buildName = userPrompt ? userPrompt.slice(0, 20) : "Image Model";

            if (engineRef.current) {
                engineRef.current.loadInitialModel(voxelData);
                setCustomBuilds(prev => [...prev, { name: buildName, data: voxelData }]);
                setCurrentBaseModel(buildName);
            }
        }
    } catch (err) {
        console.error("Image generation failed", err);
        alert("Oops! Something went wrong converting the image.");
    } finally {
        setIsGenerating(false);
    }
  };
  ...
  return (
    <div className="relative w-full h-screen bg-[#f0f2f5] overflow-hidden">
      ...
      <UIOverlay 
        ...
        onImageCreate={() => setIsImageModalOpen(true)}
